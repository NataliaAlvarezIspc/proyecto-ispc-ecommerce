...
class VentaTests(unittest.TestCase):
    def test_inicializar_venta_correctamente(self):
        cliente = Cliente(C.ID_CLIENTE, C.NOMBRE_CLIENTE, C.APELLIDO_CLIENTE, C.DNI_CLIENTE, C.USUARIO_CLIENTE, C.CLAVE_CLIENTE,
                          C.EMAIL_CLIENTE)
        envio = RetiroEnTienda()
        carrito = Carrito()
        sut = Venta(C.ID_VENTA, C.NUMERO_VENTA, C.COMPROBANTE_VENTA, C.FECHA_VENTA, cliente, C.NETO_VENTA, C.MONTO_IVA_VENTA,
                    C.NO_GRAVADO_VENTA, C.TOTAL_VENTA, envio, carrito)
        ...
        self.assertEqual(cliente, sut.obtener_cliente())
        ...
        self.assertEqual(envio, sut.obtener_envio())
        self.assertEqual(carrito, sut.obtener_carrito())
